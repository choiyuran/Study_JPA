...
//        List<Member> content = results.getResults();
    ...
    public void findDtoByField() throws Exception {
        List<MemberDto> result = queryFactory
                .select(Projections.fields(MemberDto.class,     // getter, setter 필요없고 필드에 바로 꽂힘
                        member.username, member.age))
                .from(member)
                .fetch();
        for (MemberDto memberDto : result) {
    ...
    public void findUserDtoByField() throws Exception {
        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class,
                        member.username.as("name"), member.age))            // member.username뒤에 as로 name을 지정해주지 않으면 null 값이 들어가고, 지정해주면 값이 들어감
                .from(member)
                .fetch();
    ...
    public void findUserDto() throws Exception {
        ...
        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class,
                        ...
                        // sub쿼리에는 ExpressionUtils로 감싸서 as를 사용해야 한다??
    ...
    public void dynamicQuery_whereParam() throws Exception{
        String usernameParam = "member1";
        Integer ageParam = 10;          // null로 지정해두면 where절에서 무시 당하고 조건에 들어가지 않음(username만 조건에 들어감)
...
//        return ageCond == null ? null : member.age.eq(ageCond);
    ...
    // bulk연산을 수행한 후 영속성 컨텍스트를 비워주지 않으면 db와 데이터 차이가 나게 됨
    // bulk연산 후에는 영속성 컨텍스트를 비워주자
    @Test
    public void bulkUpdate() throws Exception{
        // memebr1 = 10 -> DB member1
        // memebr2 = 20 -> DB member2
        // member3 = 30 -> DB member3
        // member4 = 40 -> DB member4

        // 영속성 컨텍스트를 무시하고 쿼리가 바로 나감
        // 즉, 영속성 컨텍스트의 상태와 db의 상태가 달라짐
        ...
        // 영속성 컨텍스트를 초기화를 해주면 db의 데이터와 일치하게 됨
        em.flush();
        em.clear();
        // 1 member1 = 10 -> 1 DB 비회원
        // 2 member2 = 20 -> 2 DB 비회원
        // 3 member3 = 30 -> 3 DB memebr3
        // 4 member4 = 40 -> 4 DB member4

        // 영속성 컨텍스트가 우선권을 가져서 출력을 하면 영속성 컨텍스트에 남아있는 데이터를 가져오게 됨(em.flush()와 em.clear()가 없는 경우)
        List<Member> result = queryFactory
                .selectFrom(member)
                .fetch();
    ...
    public void bulkAdd() throws Exception{
        long count = queryFactory
                .update(member)
                .set(member.age, member.age.multiply(2))        // multiply는 곱하기
                .execute();
    }
    ...
    public void bulkDelete() throws Exception{
        long count = queryFactory
                .delete(member)
                .where(member.age.gt(18))               // 18보다 나이가 많은 사람을 삭제(greater than)
                .execute();
    }
    ...
    public void sqlFunction2() throws Exception{
        List<String> result =  queryFactory
                .select(member.username)
                .from(member)
//                .where(member.username.eq(Expressions.stringTemplate("function('lower', {0} )", member.username)))        lower를 직접 만들어서 사용한 것
                .where(member.username.eq(member.username.lower()))         // lower가 내장되어 있음
                .fetch();
        for (String s : result) {
            System.out.println("s = " + s);
